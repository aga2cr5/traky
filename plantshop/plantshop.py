...
class PlantShop():
    ...
    def fillStock(self, plant, quantity=1, price=1):
        ...
        if plant in self.stock:
            plant_quantity = self.stock[plant][0]
            plant_price = self.stock[plant][1]
            self.stock[plant] = (plant_quantity + quantity, plant_price)
        else:
            self.stock[plant] = (quantity, price)
    ...
    def sell(self, plant, pieces=1):
        ...
        new_quantity = plant_quantity - pieces
        self.stock[plant] = (new_quantity, plant_price)

        price = self.stock[plant][1] * pieces

        if plant in self.onSale:
            salePrice = price - ((self.discount / 100) * price)
            self.assets += float(salePrice)
            return
        self.assets += float(price)


    def putOnSale(self, plant):
        """Add a plant to items on sale."""
        self.onSale.add(plant)


    def removeFromSale(self, plant):
        """Remove a plant from items on sale."""
        self.onSale.remove(plant)

    def available(self):
        """Return a list of plants that are not sold out."""
        available_plants = []

        for plant in self.stock:
            stockinfo = self.stockInfo(plant)
            if stockinfo[0] > 0:
